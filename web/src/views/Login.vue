  ...
  <div class="flex min-h-screen items-center justify-center bg-slate-900 p-4">
    <div class="w-full max-w-md">
      ...
      <div class="mb-8 text-center">
        ...
        <h1 class="text-2xl font-bold text-slate-100">{{ t('login_title') }}</h1>
        <p class="mt-1 text-sm text-slate-400">{{ t('login_subtitle') }}</p>
      </div>
      ...
      <div v-if="githubLoading" class="card text-center py-8">
        ...
        <p class="mt-3 text-slate-300">{{ t('login_completing') }}</p>
      </div>
      ...
      <div v-else-if="cliMode" class="card space-y-4">
        <div class="text-center">
          ...
          <h2 class="mt-2 text-lg font-semibold text-slate-100">{{ t('login_success') }}</h2>
          <p class="mt-1 text-sm text-slate-400">{{ t('login_welcome') }} <span class="font-medium text-slate-200">{{ cliUsername }}</span></p>
        </div>

        <div>
          <label class="mb-1.5 block text-sm font-medium text-slate-300">{{ t('login_your_token') }}</label>
        ...
        >
          ...
          {{ cliTokenCopied ? t('copied') : t('login_copy_token') }}
        </button>

        <div class="rounded-lg border border-slate-700 bg-slate-800/50 p-3">
          <p class="text-xs text-slate-400">{{ t('login_cli_hint') }} <code class="text-green-400">echome login</code>.</p>
        </div>
      ...
      <div v-else class="card space-y-5">
        ...
        <div class="relative">
          ...
          <div class="relative flex justify-center text-xs">
            ...
            >
              {{ showManual ? t('login_hide_token') : t('login_or_token') }}
            </button>
          </div>
        </div>
        ...
        <form v-if="showManual" class="space-y-4" @submit.prevent="handleTokenLogin">
          ...
          >
            ...
            {{ loading ? t('login_connecting') : t('login_connect') }}
          </button>
        </form>
        ...
        <div class="pt-2">
          ...
          >
            {{ showAdvanced ? '▾ ' + t('login_hide_advanced') : '▸ ' + t('login_advanced') }}
          </button>
          <div v-if="showAdvanced" class="mt-2">
            ...
            />
            <p class="mt-1 text-xs text-slate-500">{{ t('login_hub_url_hint') }}</p>
          </div>
        </div>
      </div>
