...
>
> 以下内容是写作示例，不代表 EchoMe 当前部署配置或所有用户都应采用的规范。类型、状态与加载边界见 [记忆模型](memory-model.md)。

## 记忆类型（Type）与优先级

EchoMe 的 10 种记忆类型按**渲染优先级**排列如下——先按类型顺序、再按类型内 priority 选择；条目超过剩余预算仍会被跳过：

| 优先级 | Type | 含义 | 影响什么 |
|--------|------|------|----------|
| 1 (最高) | `identity` | 人格基线 | AI 的角色定位、语气、性格 |
| 2 | `guardrail` | 红线与禁忌 | 绝对不能做的事 |
| 3 | `reasoning` | 思维框架 | 如何分析问题、做取舍 |
| 4 | `method` | 工作流程 | 解决问题的方法论和标准流程 |
| 5 | `stack` | 技术偏好 | 语言、框架、工具链选择 |
| 6 | `style` | 交互风格 | 输出格式、沟通方式、长度偏好 |
| 7 | `decision` | 决策记录 | 历史技术决策和原因 |
| 8 | `context` | 领域知识 | 业务逻辑、架构上下文 |
| 9 | `template` | 代码模板 | 可复用的代码片段 |
| 10 (最低) | `project` | 项目背景 | 特定项目的介绍和约定 |
...
思维框架——教 AI 分析问题的路径。这是让 AI "变强"最有效的类型。
...
遇到复杂问题时按这个顺序：
1. 理解：先复述问题确认理解正确
2. 拆解：把大问题拆成可独立解决的小问题
3. 方案：列 2-3 个方案，说明 trade-off
4. 选择：推荐一个但让我最终决定
5. 执行：选定后直接动手，不要反复确认
```
...
| Layer | 何时加载 | 适合放什么 |
|-------|----------|-----------|
| **L0** | 每次对话都加载 | identity, guardrail, 核心 method, 全局 stack |
| **L1** | 进入特定项目时加载 | 项目规范、项目技术栈、项目 method |
| **L2** | AI 按需搜索 | 大段知识、template、历史 decision |
