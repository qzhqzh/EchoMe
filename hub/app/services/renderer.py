...
规则：
1. **会话启动自检**：收到用户第一条任务消息后，调用 MCP tool `echome_search_summary` 查询与当前任务相关的身份偏好、交互风格、代码规范、项目规范和历史决策摘要。
2. **首轮规范确认**：如果摘要中有相关规范，调用 `echome_get_memories` 读取必要全文，然后用 1-3 句简短复述本次会遵守的关键规范，再开始执行任务。
3. **后续按需触发**：同一会话后续不要每轮都查。只有当用户提到偏好/规范/历史决策/项目约定，任务跨模块或高风险，出现不确定约定，或用户说“按老规矩/继续/记住/以后/永远/always”时，再调用 `echome_search_summary`。
4. **精读取舍**：只对当前任务相关的摘要条目调用 `echome_get_memories` 读取全文；调用时必须传摘要里的 UUID，不要只传编号。
5. **没有命中就停止**：如果摘要中没有相关记忆，不要为了凑结果继续扩大搜索；直接说明未找到相关记忆并继续处理当前任务。
6. **明确小问题可直搜**：对非常明确的小范围问题，可以直接调用 MCP tool `echome_search`；不确定项目约定时优先用 `echome_search_summary` 而不是假设。
7. 用户说"记住/以后/永远/always"时，必须调用 MCP tool `echome_remember`。
8. 写入记忆时 type 只能是：identity, guardrail, reasoning, method, stack, style, decision, context, template, project。写入前判断记忆范围：如果是通用型（适用于所有项目）则不传 project；如果是项目相关（仅适用于特定项目），则必须指定有效项目（project 参数）。
9. **project 类型记忆需要关联项目**：写入 project 类型记忆前，先调用 MCP tool `echome_list_projects` 查看已有项目；若目标项目不存在，调用 MCP tool `echome_create_project` 创建。提交时必须带上 `project` 参数（项目名称），`suggested_layer="L1"`，状态默认为 ai_review。
